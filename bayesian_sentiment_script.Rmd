...
We initially specified the initial state as `x[1,j] ~ dnorm(mu[j], tau.eta[j])`. In practice, this centered parameterization led to slow mixing and occasional multimodal-looking trace behavior in preliminary runs. 
To improve MCMC efficiency, we therefore use a non-centered parameterization for the latent state:
`x[1,j] <- mu[j] + eps[1,j] * sigma.eta[j]`, with `eps[1,j] ~ dnorm(0,1)`. This reparameterization is 
mathematically equivalent but typically samples more efficiently in hierarchical state-space models.
...
gx <- x
gmu_theta <- mu_theta
gsigma_theta <- sigma_theta
gmu_log_sigma.eta<-mu_log_sigma.eta
gsigma_log_sigma.eta<-sigma_log_sigma.eta
gmu_log_sigma<-mu_log_sigma
gsigma_log_sigma<-sigma_log_sigma
geps <- eps
detach.jags()
...
summary(gx)
summary(gmu_theta)
summary(gsigma_theta)
summary(gmu_log_sigma.eta)
summary(gsigma_log_sigma.eta)
summary(gmu_log_sigma)
summary(gsigma_log_sigma)
summary(geps)
```
...
### Homoscedastic (constant-variance) Model

The general model but without considering $n_{tj}$.
```{r}
BUGS.Cvar_model <- "
model
{
...
Train model and traceplot.
```{r}
Cvar_model <-jags(data, parameters.to.save=gparameters,
 		   model=textConnection(BUGS.Cvar_model),
		   n.iter=(Iter*Thin+Burn),n.burnin=Burn, n.thin=Thin, n.chains=Chain)


traceplot(Cvar_model, mfrow = c(2,2), varname = c("mu", "sigma", "sigma.eta", "theta", "mu_theta", "sigma_theta", 
"mu_log_sigma.eta", "sigma_log_sigma.eta", 
"mu_log_sigma", "sigma_log_sigma"))
```
...
Save parameters and show its summary.
```{r}
attach.jags(Cvar_model)
cmu <- mu
colnames(cmu) <- colnames(C2)
csigma <- sigma
csigma.eta <- sigma.eta
ctheta <- theta
cx <- x
cmu_theta <- mu_theta
csigma_theta <- sigma_theta
cmu_log_sigma.eta<-mu_log_sigma.eta
csigma_log_sigma.eta<-sigma_log_sigma.eta
cmu_log_sigma<-mu_log_sigma
csigma_log_sigma<-sigma_log_sigma
ceps <- eps
detach.jags()
...
par(mfrow = c(2, 1))
plot_density(gmu_theta, name = "mu_theta")
plot_density(gsigma_theta, name = "sigma_theta")

par(mfrow = c(2, 1))
plot_density(gmu_log_sigma.eta, name = "mu_log_sigma.eta")
plot_density(gsigma_log_sigma.eta, name = "sigma_log_sigma.eta")

par(mfrow = c(2, 1))
plot_density(gmu_log_sigma, name = "mu_log_sigma")
plot_density(gsigma_log_sigma, name = "sigma_log_sigma")

```
Exploratory residual diagnostic: we plot the difference between the posterior mean of the latent state ($x_post_mean$) and the observed aggregated score $y$ against the effective weight $n$. Under the observation model, residual dispersion should decrease as $n$ increases (heteroskedasticity driven by 1/$n$).
...
```{r}
# Posterior predictive checks (PPC) and related diagnostics.
# 'posterior' corresponds to the standard PPC (replicated y ~ p(y | x, sigma) with (x, sigma) drawn from the posterior).
# 'conditional' fixes x to its posterior median to isolate the observation noise component.
# 'forward' simulates x from the fitted state equation to assess whether the dynamic prior generates realistic trajectories.
# Results reported in the paper correspond to the standard posterior PPC ('posterior').
...
ppc_category <- function(y, n, draws, j,
                         type = c("posterior", "conditional", "forward"),
                         nsim = 500,
  ...
  if (type %in% c("posterior", "conditional")) {
    for (k in seq_along(sims)) {
      s <- sims[k]
      mu_t <- if (type == "posterior") x_draws[s, idx, j] else x_hat[idx]
...
ppc_all_r2jags <- function(fit, y, n,
                           ...
                           type = c("posterior", "conditional", "forward"),
                           nsim = 500, max_draws = 2000, seed = 123) {
  type <- match.arg(type)
...
res_ppc <- ppc_all_r2jags(general_model, y, n, type="posterior")
res_ppc$summary

dir.create("tables", showWarnings = FALSE)
saveRDS(res_ppc, "tables/res_ppc.rds")
...
# map t -> week start dates (same as exploratory figure)
# df must exist; this uses week_start
df_time <- df %>%
  distinct(temporal_id, week_start) %>%
  arrange(temporal_id)

x_dates <- as.Date(df_time$week_start)
stopifnot(length(x_dates) == N)

df_yx <- tibble(
  t = rep(seq_len(N), times = M),
  x = rep(x_dates, times = M),
...
# coverage bars (n_tj) in the same panel, globally rescaled
ymin <- -0.75
ymax <-  0.50
yrng <- ymax - ymin

# cap n at p99 for robustness (global)
n_cap_val <- quantile(df_yx$n, probs = 0.99, na.rm = TRUE)
...
df_yx <- df_yx %>%
  mutate(
    bar_top = ymin + n01 * bar_height
  )

p_yx <- ggplot(df_yx, aes(x = x)) +
  # coverage bars behind everything
  geom_linerange(
    aes(ymin = ymin, ymax = bar_top),
    color = "grey60", alpha = 0.5, linewidth = 1.4
  ) +
  # uncertainty band + latent mean + observed points
  ...
  facet_wrap(~ category, ncol = 2, labeller = labeller(category = pretty_map), drop = FALSE) +
...
```{r}
df <- df %>% mutate(temporal_id = temporal_id)
# Map temporal_id -> week_start (or day) date
df_time <- df %>%
  distinct(temporal_id, week_start) %>%   # if time_mode == "week"
  arrange(temporal_id)

# Create a Date vector of length N with the start date of each time window
x_dates <- as.Date(df_time$week_start)
...
```{r}
# x_draws: array [S, N, M]  (S draws, N time points, M categories)
summarize_x_draws <- function(x_draws, x_dates, labels_M) {
  stopifnot(length(dim(x_draws)) == 3)
  S <- dim(x_draws)[1]
  N <- dim(x_draws)[2]
  M <- dim(x_draws)[3]

  stopifnot(length(x_dates) == N)
  stopifnot(length(labels_M) == M)

  # summaries as matrices [N, M]
  x_mean <- apply(x_draws, c(2, 3), mean, na.rm = TRUE)
  x_q05  <- apply(x_draws, c(2, 3), quantile, probs = 0.05, na.rm = TRUE)
  x_q95  <- apply(x_draws, c(2, 3), quantile, probs = 0.95, na.rm = TRUE)

  # long dataframe: N*M rows
  df_x <- tibble(
    t        = rep(seq_len(N), times = M),
    x        = rep(x_dates, times = M),
    category = rep(labels_M, each = N),
...
# Time axis mapping
df_time <- df %>%
  distinct(temporal_id, week_start) %>%   # weekly
  arrange(temporal_id)

x_dates <- as.Date(df_time$week_start)
stopifnot(length(x_dates) == N)

# Long data for y and n
df_long <- tibble(
  t        = rep(seq_len(N), times = M),
  x        = rep(x_dates, times = M),
  category = rep(labels_M, each = N),
  ...
  mutate(
    category = factor(category, levels = labels_M)
  )

# Summaries for BOTH models
df_x_hetero <- summarize_x_draws(general_model$BUGSoutput$sims.list$x, x_dates, labels_M) %>%
  mutate(model = "Heteroscedastic (scaled by n)")

df_x_homo <- summarize_x_draws(Cvar_model$BUGSoutput$sims.list$x, x_dates, labels_M) %>%
  mutate(model = "Homoscedastic (constant variance)")

df_x_both <- bind_rows(df_x_hetero, df_x_homo) %>%
  mutate(model = factor(model, levels = c(
    "Heteroscedastic (scaled by n)",
    "Homoscedastic (constant variance)"
  )))

# Convert x05/x95 into long format for plotting as lines
df_x_bounds <- df_x_both %>%
  select(t, x, category, model, x05, x95) %>%
  pivot_longer(
    cols = c(x05, x95),
    names_to = "bound",
    values_to = "value"
  ...
  mutate(
    ...
    bound = factor(bound, levels = c("Lower 5%", "Upper 95%"))
  )


# Rescale n into vertical bars (as background)
ymin <- -0.7
ymax <-  0.5
yrng <- ymax - ymin
...
# X limits
xmax_data <- df_long %>% pull(x) %>% max(na.rm = TRUE)
x_margin  <- 1
xmax_plot <- xmax_data + x_margin

p_x_compare <- ggplot() +

  # n(t,j) bars in the background (grey)
  geom_linerange(
    data = df_long,
    aes(x = x, ymin = ymin, ymax = bar_top),
    color = "grey70", alpha = 0.35, linewidth = 1.2
  ) +

  # Posterior interval bounds as lines (no ribbons, no mean)
  geom_line(
    data = df_x_bounds,
    aes(x = x, y = value, color = model, linetype = bound),
    linewidth = 0.9
  ) +

  # observed y as points
  geom_point(
    data = df_long,
    aes(x = x, y = y),
    color = "black",
  ...
  coord_cartesian(
    ylim = c(ymin, ymax),
    xlim = c(min(df_long$x, na.rm = TRUE), xmax_plot)
  ) +
...
ggsave("figures/fig_x_compare_hetero_vs_homo.pdf",
       p_x_compare,
       width = 10.5, height = 7.5, device = cairo_pdf)
```
In order to validate the relation this width has with $1/\sqrt n_{tj}$ we define the following model: 
$$
w_{tj} = \beta_0 + \beta_1 \frac{1}{\sqrt n_{tj}} + \beta_2I_{homo}+\beta_3 I_{homo} \frac{1}{\sqrt n_{tj}} + \text{category effects}
$$
Where $w_{tj}=x95_{tj}-x05_{tj}$ and $I_{homo}=1$ for the Homoscedastic model and $I_{homo}=0$ for the Heteroscedastic model.
```{r}
# define width and add n to the df
df_width <- df_x_both %>%
  mutate(width90 = x95 - x05) %>%
  left_join(df_long %>% select(t, category, n), by = c("t", "category"))

# the relation
df_width <- df_width %>%
  mutate(inv_sqrt_n = 1 / sqrt(pmax(n, 1))) # log_n = log1p(n)

# Spearman corr by category and model
cor_by_cat <- df_width %>%
  group_by(model, category) %>%
  summarise(
    spearman_rho = cor(inv_sqrt_n, width90, method = "spearman", use = "complete.obs"),
    pearson_r    = cor(inv_sqrt_n, width90, method = "pearson",  use = "complete.obs"),
    .groups = "drop"
  )

cor_by_cat # the correlation is clearly higher for the Heteroscedastic model

summary(df_width$width90) # to put the magnitude of the coefficients into context
sd(df_width$width90, na.rm = TRUE) 
# to see how many standard deviations the width increases per unit of inv_sqrt_n

fit <- lm(width90 ~ inv_sqrt_n * model + category, data = df_width)
summary(fit)

# A one-unit increase in $1/\sqrt n_{tj}$ widens the 90% credible interval 
# by 0.0978 (\beta_1) in the heteroscedastic model, compared to 0.0412 (\beta_1-\beta_3) 
# in the homoscedastic variant, implying that the heteroscedastic specification 
# responds more strongly to changes in information density.

# to show in the appendix
coef_tab <- broom::tidy(fit) %>%
  filter(!grepl("^category", term)) %>%
  mutate(
    term = recode(term,
      "inv_sqrt_n" = "1/sqrt(n_tj)",
      "modelHomoscedastic (constant variance)" = "I_homo",
      "inv_sqrt_n:modelHomoscedastic (constant variance)" = "I_homo × 1/sqrt(n_tj)"
    ),
...
saveRDS(res_width_reg, "tables/res_width_reg.rds")
```
## Additional
...
### Bimodality and correlation check

In Gaussian state-space models, persistence and variance parameters ($\theta$, $\sigma$, $\sigma_\eta$)can sometimes exhibit strong posterior correlations due to the standard observation-noise vs state-noise trade-off. In some datasets or model variants (especially under pooled specifications), this may also lead to bimodal marginal posteriors, reflecting multiple high-probability parameter regimes that provide similarly good fits.

The following diagnostic code is included as a precautionary check to verify that posterior marginals are unimodal and
consistent across chains, and to quantify posterior correlations between key parameters.

In the current analysis, we do not observe problematic bimodality or unusually strong correlations, but these checks are
kept for transparency and reproducibility.
```{r}
# Posterior densities by chain for key pooled parameters (theta, sigma, sigma.eta).
# This is a precautionary diagnostic: in some datasets/model variants these parameters may show strong correlations
# or apparent bimodality due to the observation-noise vs state-noise trade-off in state-space models.
# Plotting densities by chain helps check whether any bimodality is consistent across chains (posterior feature)
# or driven by chain-to-chain differences (potential convergence issue).
density_by_chain <- function(param_m, param_name){
  df_theta <- data.frame(
    value = c(as.numeric(param_m[[1]]),
              as.numeric(param_m[[2]]),
              as.numeric(param_m[[3]])),
    chain = factor(rep(1:3, times = sapply(param_m, length)))
  )
  
  ggplot(df_theta, aes(x = value, color = chain)) +
    geom_density(linewidth = 1.0) +
    labs(
      title = "Posterior density by chain",
      x = param_name,
      y = "Density",
      color = "Chain"
    ) +
...
m <- as.mcmc(simple_model)
density_by_chain(m[, "theta"], "theta")
density_by_chain(m[, "sigma"], "sigma")
density_by_chain(m[, "sigma.eta"], "sigma.eta")

# Correlation diagnostics:
# - Simple (pooled) model: correlation matrix for (theta, sigma, sigma.eta).
# - General (hierarchical) model: within-category correlations for the same parameters.
# These summaries help quantify the strength of the posterior trade-off between observation noise and state innovation.

# for the simple model: correlation matrix (pooled parameters)
S_simple <- cbind(
  theta     = stheta,
  sigma     = ssigma,
  sigma_eta = ssigma.eta
)
round(cor(S_simple, use = "complete.obs"), 3)

# for the general model: correlation
cor_by_category <- function(gtheta, gsigma, gsigma_eta, labels = NULL) {
  M <- ncol(gtheta)
  if (is.null(labels)) labels <- paste0("cat_", seq_len(M))
  labels <- as.character(labels)

  out <- data.frame(
    category = labels,
    cor_theta_sigma     = NA_real_,
    cor_theta_sigma_eta = NA_real_,
    cor_sigma_sigma_eta = NA_real_
  )
  ...
  for (j in 1:M) {
    S <- cbind(
      theta     = gtheta[, j],
      sigma     = gsigma[, j],
      sigma_eta = gsigma_eta[, j]
    )
    C <- cor(S, use = "complete.obs")
    out$cor_theta_sigma[j]     <- C["theta", "sigma"]
    out$cor_theta_sigma_eta[j] <- C["theta", "sigma_eta"]
    out$cor_sigma_sigma_eta[j] <- C["sigma", "sigma_eta"]
  }

  out
}

cor_table <- cor_by_category(gtheta, gsigma, gsigma.eta, labels = labels_M)
cor_table
```

### Some traceplots

MCMC traceplots for concrete general model parameters.
```{r}
